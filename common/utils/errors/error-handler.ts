...
import {logError, logUnknownError} from '../logging/logger';
...
const handleGenericError = ({operationName, error}: ErrorHandlerGeneric) => {
  ...
  const message = ERROR_MESSAGE.UNRECOGNIZED;
  logError({
    message,
    operationName,
    error,
...
const handlePrismaError = ({
  ...
  logError({
    message: ERROR_MESSAGE.UNRECOGNIZED,
    operationName,
    error,
    referenceId,
  });
  return Failure.internalServer(
    `${ERROR_MESSAGE.UNRECOGNIZED}. Reference Id: ${referenceId}`,
  );
};
